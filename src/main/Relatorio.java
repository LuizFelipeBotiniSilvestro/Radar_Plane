...
public class Relatorio {
	...
	public static void tempColisao(TAviaoTableModel planeTableModel, JTextArea txtReport) {
        ...
        try {
            tempoMinimo = Double.parseDouble(JOptionPane.showInputDialog("Distância mínima para alertar a colisão (Segundos):"));
        ...
        for (int i = 0; i < planeTableModel.getRowCount(); i++) {
            ...
            for (int j = i; j < planeTableModel.getRowCount(); j++) {
                ...
                if (info.getDiferencaTempo() <= tempoMinimo) {
                	...
                	stringBuilder.append("O avião: " +aviaoA.getCodigo()  + " está em rota de colisão com o avião " + aviaoB.getCodigo() + "\n");
	...
	public static void distanciaAvioes(TAviaoTableModel planeTableModel, JTextArea txtReport) {
		...
		try {
			distancia = Double.parseDouble(JOptionPane.showInputDialog("informar a distância mínima entre as aeronaves! (km)"));
		...
		for (int i=0; i < planeTableModel.getRowCount(); i++ ) {
			...
			for (int j=0; j < planeTableModel.getRowCount(); j++ ) {
				...
				if (distanciaCalc <= distancia) {
					...
					stringBuilder.append("O avião: " + aviaoA.getCodigo() +" está abaixo da distância mínima do avião " + aviaoB.getCodigo() + "\n");
				}
			}
		}
